...
//	private static final String FACE = "f";
	...
	public static RawModel loadOBJ(String filename, Loader loader) {
		...
		File objFile = new File(Main.fileHandler.getModel(filename));

		// load up files
		try {
			objectFileReader = new FileReader(objFile);
		} catch (FileNotFoundException e) {
			e.printStackTrace();
			return null;
		}

//		HashMultimap<String, Vector> infoTypes = HashMultimap.create();
//		try (BufferedReader br = Files.newBufferedReader(Paths.get(filename))) {
//			for (String line : br.lines().collect(Collectors.toList())) {
//				List<String> components = Arrays.asList(line.split(SPLIT_REGEX));
//				if ((components.size() == 3 || components.size() == 4) &&
//						Arrays.asList(VECTOR, TEXTURE, NORMAL, FACE).contains(components.get(0))) {
//					infoTypes.put(components.get(0), Utils.listToVector(components.subList(1, components.size())));
//				}
//			}
//		} catch (IOException e) {
//			e.printStackTrace();
...
//		}
		...
		BufferedReader reader = new BufferedReader(objectFileReader);
		String line;
